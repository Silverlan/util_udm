...
std::shared_ptr<udm::Data> udm::Data::Create() { return Create("", 0); }
...
bool udm::Data::DebugTest()
{
	try {
		...
		auto structArray = udmData.AddArray("structArray", structDef, 10);
		for(auto i = 0; i < 10; ++i)
			structArray[i] = TestStruct {5.f, Vector3 {3, 3, 3}, 2, 0.34f};

		auto aCompressedStructArray = udmData.AddArray("compressedStructArray", structDef, 10, ArrayType::Compressed);
		for(auto i = 0; i < 10; ++i)
			aCompressedStructArray[i] = TestStruct {5.f, Vector3 {3, 3, 3}, 2, 0.34f};

		std::vector<TestStruct> structuredData = {TestStruct {5.f, Vector3 {3, 3, 3}, 2, 0.34f}};
		auto aCompressedStructArray2 = udmData.AddArray("compressedStructArray2", structDef, structuredData, ArrayType::Compressed);
