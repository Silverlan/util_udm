...
namespace udm {
	class AsciiReader {
	  ...
	  private:
		enum class BlockResult : uint8_t { EndOfBlock = 0, EndOfFile };
...
udm::AsciiException::AsciiException(const std::string &msg, uint32_t lineIdx, uint32_t charIdx) : Exception {msg + " in line " + std::to_string(lineIdx + 1) + " (column " + std::to_string(charIdx + 1) + ")"}, lineIndex {lineIdx}, charIndex {charIdx} {}

udm::Type udm::ascii_type_to_enum(const std::string_view &type)
{
	// Note: These have to match enum_type_to_ascii
	static std::unordered_map<std::string_view, Type> namedTypeToEnum = {{"nil", Type::Nil}, {"string", Type::String}, {"utf8", Type::Utf8String}, {"int8", Type::Int8}, {"uint8", Type::UInt8}, {"int16", Type::Int16}, {"uint16", Type::UInt16}, {"int32", Type::Int32},
	...
	if(is_float_based_type(type)) {
		std::visit(
		  [this, &outData](auto tag) {
			  ...
			  auto fixQuatComponentOrder = [](Quat &v) { v = {v.x, v.y, v.z, v.w}; };
	...
	case Type::ArrayLz4:
		{
			...
			ReadValueList(
			  valueType,
			  [this, &fReadValue, &a, &numValues]() -> bool {
...
struct MemoryData : public udm::IFile {
	MemoryData() = default;
	...
	virtual size_t Tell() override { return m_pos; }
...
std::string_view udm::AsciiReader::ReadString() { return ReadString(m_file->ReadChar()); }
